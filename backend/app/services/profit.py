...
def compute_split(
    ...
    from revenue. Sales tax we paid on the buy IS part of total_cost.

    net_profit = (sale_price - sales_tax_collected) - (retail_cost + sales_tax_paid)
                 - fees - shipping_out

    If investor funded the buy: investor recovers total_cost first, then takes
    profit_share_pct of net_profit. Owner gets the rest.
    """
    revenue = (sale_price or 0) - (sales_tax_collected or 0)
    total_cost = (retail_cost or 0) + (sales_tax_paid or 0)
    fees = fees or 0
